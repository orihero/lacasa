 ...
 * tree (wired into QueryClientProvider in App.tsx) and cannot call a hook,
 * so there is no `useAuth()` it could reach for directly. This keeps the two
 * files decoupled — queryClient.ts doesn't import React or auth.tsx, and
 * auth.tsx doesn't import react-query — while still letting a 401 anywhere
 * in the app trigger the same recovery path the mount-time session check in
 * AuthProvider already has, not just the first request of the session.
 */
type Listener = () => void;
