/**
 * src/lib/queryKeys — every react-query key factory in one place, so a cache
 ...
 * the same.
 *
 * ALL FOUR FEATURES ARE DECLARED HERE UP FRONT, including the ones whose data
 * hooks belong to other screens. That is the point: after an admin approves an
 * application, the applications list, the users list AND the overview's counts
 * are all stale, and the mutation has to be able to invalidate all three by key
 * without importing anything from the screens that own them. Screens import
 * from this file and must not edit it.
 *
 ...
 * to fold to the same key, or the rail's badge subscription and a screen's own
 * query would fetch the same page twice.
 */
...
export const queryKeys = {
  overview: {
    ...
    // every other key here is called, and a lone non-callable would be the one
    // that gets `queryKeys.overview.summary` (the function itself, as a key)
    // passed by mistake.
  ...
  applications: {
    ...
    // contract says an omitted status means pending, so a call with no filters
    // and a call with `{ status: "pending" }` are the same request and must be
