/**
 * src/data/useApplications — the realtor application queue's data layer:
 * one paged read and one decision mutation, both against the typed `admin`
 * resource of @lacasa/api-client.
 *
 * WHY useInfiniteQuery AND NOT useQuery (unlike apps/console's useLeads, which
 * this is otherwise modelled on): /api/admin/applications is keyset-paged on
 * `(createdAt desc, id desc)` and hands back an opaque `nextCursor`. There is
 * no total and no page number, so the only shape the endpoint supports is
 * "give me the next slice after this one" — which is exactly `pageParam`. That
 * is also why queryKeys.applications.list() deliberately excludes the cursor:
 * folding it into the key would give every page its own cache entry and the
 * accumulated list would reset on every fetch.
 *
...
import { queryKeys, type ApplicationsStatusFilter } from "./queryKeys";
 ...
 * reports is a number this app decided, not one it inferred from whatever came
 * back — and so raising it is a one-line change here instead of a deploy.
 */
...
/**
 * The query options split out of the hook so the request they describe — path,
 * status, page size, cursor threading — can be asserted against a faked
 * transport without mounting React. That is not a cosmetic seam in this
 * workspace: apps/web pins React 18 and claims the hoisted copy at the repo
 * root, so @tanstack/react-query (hoisted alongside it) cannot render inside a
 * test at all here; see src/test/render.tsx for the full writeup. Without this
 * split the wiring below would be untestable rather than merely awkward.
 */
  ...
  return {
    ...
    queryFn: ({ pageParam }: { pageParam: string | null }) =>
      apiClient.admin.listApplications({
        // ApplicationsStatusFilter (queryKeys.ts) and AdminApplicationStatus
        ...
        // The first page must send NO cursor at all: `?cursor=` would mean
        // "the empty cursor" rather than "from the beginning", so the null
        // initial pageParam is converted back to undefined for the resource,
        // which drops undefined query keys.
    ...
    // contract's own sentinel straight through rather than re-deriving
    // "is there more?" from the row count, which would be wrong on a final
    // page that happens to be exactly APPLICATIONS_PAGE_SIZE long.
 ...
 * means another admin decided this application between the queue being
 * rendered and the button being pressed. Nothing is broken and nothing needs
 * retrying — the local view is simply out of date.
 *
 * The code is compared through `string` on purpose. @lacasa/domain's
 * ERROR_CODES was collected from the API's call sites before /api/admin
 * existed and does not list `not_pending`, while lib/apiClient.ts casts
 * whatever code the body carried into that union — so `error.code` is typed
 * more narrowly than it actually is, and comparing against the union directly
 * would be a compile error about a value the server genuinely sends.
 */
...
/**
 * The mutation's options, taking the cache it invalidates as an argument
 * rather than reaching for `useQueryClient()` itself — same reason
 * applicationsQueryOptions is split out (react-query cannot render in this
 * workspace's tests), and here it buys more: which keys a decision
 * invalidates is the part of this file most likely to rot silently, since a
 * missed key shows up as a stale count on a screen somebody else owns rather
 * than as a failure on this one. A test can hand this a stub client and assert
 * the keys directly.
 */
export function decideApplicationMutationOptions(
  ...
  return {
    ...
    onSuccess: () => {
      ...
      // directory is showing — queryKeys.ts declares all four features up
      // front precisely so this mutation can say so without importing from a
      // screen another agent owns.
    ...
    onError: (error) => {
      ...
      // screen is provably wrong and refetching it is the fix. Every other
      // error leaves the cache alone: the decision did not land, the list is
      // still accurate, and silently refetching would hide that fact behind a
      // spinner.
