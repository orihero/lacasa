 ...
 * which matters here because this workspace cannot currently mount
 * @tanstack/react-query in a test at all (it is hoisted to the repo root and
 * binds the root's React 18 while this app renders with its own React 19 —
 * see src/test/render.tsx's header for the full writeup).
 *
...
/**
 * The server-side filters, narrowed past `queryKeys.AuditFilters` (whose
 * `type` is a bare `string`, since that module cannot import the api-client's
 * enum without pulling the whole contract into a key factory). Structurally
 * assignable to it, so the key factory still takes this verbatim.
 *
...
export function auditQueryOptions(filters: AuditFilters = {}) {
  return {
    ...
    // keys while serialising null as the string "null".
